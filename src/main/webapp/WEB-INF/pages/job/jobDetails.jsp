...
<body>
	<div id="body">
		<div id="header">
			<div class="wrapper">
				...
				<ul class="reset" id="navheader">
					...
					<li><a style="color: #12CD57" href="${ctx}/resumeCreate"
						rel="nofollow">我的简历</a></li>
		...
		<div id="container">
			<div class="clearfix">
				<div class="content_l">
					<dl class="job_detail">
						...
						<dd class="job_request">
							<span class="red"> <c:if
									test="${job.jobInfo.salaryRange==1 }">2001-4000</c:if> <c:if
									test="${job.jobInfo.salaryRange==2 }">4001-6000</c:if> <c:if
									test="${job.jobInfo.salaryRange==3 }">6001-8000</c:if> <c:if
									test="${job.jobInfo.salaryRange==4 }">8001-10000</c:if> <c:if
									test="${job.jobInfo.salaryRange==5 }">10001-15000</c:if> <c:if
									test="${job.jobInfo.salaryRange==6 }">15001-20000</c:if> <c:if
									test="${job.jobInfo.salaryRange==7 }">20001-30000</c:if> <c:if
									test="${job.jobInfo.salaryRange==8 }">面议</c:if>
							</span> <span>${job.jobInfo.city}</span> <span> <c:if
									test="${job.jobInfo.workExperience==1 }">应届</c:if> <c:if
									test="${job.jobInfo.workExperience==2 }">1-2年</c:if> <c:if
									test="${job.jobInfo.workExperience==3 }">2-3年</c:if> <c:if
									test="${job.jobInfo.workExperience==4 }">3-4年</c:if> <c:if
									test="${job.jobInfo.workExperience==5 }">5年</c:if> <c:if
									test="${job.jobInfo.workExperience==6 }">5年以上</c:if>
							</span> <span> ${job.highistEducation}</span> <span>${job.jobNature }</span><br />

							<%-- <div>发布时间：<fmt:formatDate value="${job.announceTime}" pattern="yyyy-MM-dd"/></div> --%>
						</dd>
						<dd class="job_bt">
							<h3 class="description">职位描述</h3>
							<p>${job.jobInfo.discription}</p>
						</dd>

						<dd class="unresume">
							<div>
								你在伯乐还没有简历呢，你可以<a href="${ctx}/resumeCreate" target="_blank"
									title="创建在线简历">创建在线简历</a>
							</div>
						</dd>
						...
						<dd>
							...
							<div style="width:100px;height:30px;background-color:#FF9A10;margin-left: 550px">
							<a style="font-size: 18px;line-height: 30px;color: #E8F2FE" 
								href="${ctx}/sendResume?jobId=${job.jobId}&companyId=${job.company.companyId}"
								onclick="test('${job.jobId}','${job.company.companyId}')"
								id="send" target="_blank"> 投递简历 </a>
							</div>
						</dd>
					</dl>
				...
				<div class="content_r">
					<dl class="job_company">
						<dt>
							<a href="${ctx }/companyhome" target="_blank"> <img
								class="b2" src="${ctx}/${job.company.logo}" width="80"
								height="80" alt="${job.company.companyName}" />
								<div>

									<h2 class="fl">
										${job.company.companyName} <img
											src="${ctx}/style/images/valid.png" width="15" height="19"
											alt="伯乐认证企业" /> <span class="dn">伯乐认证企业</span>
									</h2>
								</div>
							</a>
						</dt>
						<dd>
							<ul class="c_feature reset">
								...
								<li><span>规模</span> <c:if
										test="${job.company.companyInfo.scale==1}">小于50人</c:if> <c:if
										test="${job.company.companyInfo.scale==2 }">50-150人</c:if> <c:if
										test="${job.company.companyInfo.scale==3 }">150-300人</c:if> <c:if
										test="${job.company.companyInfo.scale==4 }">300-500人</c:if> <c:if
										test="${job.company.companyInfo.scale==5 }">500-100人</c:if> <c:if
										test="${job.company.companyInfo.scale==6 }">1000人以上</c:if></li>
					...
					<a
						...
						src="${ctx}/style/images/zhouyou.jpg" width="280" height="135" />
					</a>
				</div>
			</div>
			...
			<div id="loginToolBar">
				<div>
					<em></em> <img src="${ctx}/style/images/footbar_logo.png"
...
<script src="style/js/common.js" type="text/javascript"></script> -->
			...
			<script type="text/javascript">
				...
				if ($('#positionLat').val() && $('#positionLng').val()) {
					...
					sMyGeo.getPoint("${job.company.companyInfo.address}",
							function(sPoint) {
								if (sPoint) {
					...
					$('#mapPreview')
							.bind(
									...
									function() {
										...
										var address = $
										{
											job.company.companyInfo.address
										}
										;
										var city = $
										{
